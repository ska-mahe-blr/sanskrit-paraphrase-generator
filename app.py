...
app = Flask(__name__)
app.secret_key = 'abc'  # Needed for session
...
def process_step():
    ...
    while True:
        ...
        #if tag in ['A1','A2','A3', 'A4', 'A5', 'A6', 'A7', 
        ...
        #    return render_template('index.html', output_html=error_html)
        #else:
        #    error_html = f'<span style="color:red;">Error: Please check the tag. It is case-sensitive."{html.escape(tag)}" in input: {html.escape(match.group(0))}</span><br>'
        #    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        #        return error_html
        #    return render_template('index.html', output_html=error_html)
